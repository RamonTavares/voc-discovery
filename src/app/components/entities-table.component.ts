    ...
    parseData(data) {
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i]["review_text_enriched"] && data[i]["review_text_enriched"]["entities"].length > 0) {
                jsonObj["doc_sentiment"]["type"] = data[i]["review_text_enriched"]["sentiment"]["document"]["label"];
                jsonObj["doc_sentiment"]["score"] = data[i]["review_text_enriched"]["sentiment"]["document"]["score"] ? data[i]["review_text_enriched"]["sentiment"]["document"]["score"] : null;
                jsonObj["doc_emotions"] = data[i]["review_text_enriched"]["emotion"]["document"]["emotion"];
		    ...
		    if (obj["sentiment"]["label"])
                    	entity["sentiment"]["type"] = obj["sentiment"]["label"];
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i]["review_text_enriched"] && data[i]["review_text_enriched"]["entities"].length > 0) {
                ...
                for (var j = 0; j < doc.length; j++) {
                    ...
                    jsonObj["review_text"] = data[i]["review_text"];
                    jsonObj["summary"] = data[i]["summary"];
                    jsonObj["doc_sentiment_type"] = data[i]["review_text_enriched"]["sentiment"]["document"]["label"];
                    jsonObj["doc_sentiment_score"] = data[i]["review_text_enriched"]["sentiment"]["document"]["score"] ? data[i]["review_text_enriched"]["sentiment"]["document"]["score"] : null;
                    ...
                    jsonObj["sentiment_type"] = obj["sentiment"]["label"];
